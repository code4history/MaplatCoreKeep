...
export class MaplatMap extends Map {
    ...
    setLine(xys, stroke, layer) {
        ...
        const style = stroke != null ? new Style({
    ...
    handleGPS(launch, avoidEventForOff) {
        ...
        if (launch) {
            ...
            if (!this.geolocation) {
                const geolocation = this.geolocation = new Geolocation({tracking: true});
                // listen to changes in position
                geolocation.on('change', (evt) => { // eslint-disable-line no-unused-vars
            ...
            const source = this.getLayers().item(0).getSource();
            source.setGPSMarker();
            if (!avoidEventForOff) this.dispatchEvent(new MapEvent('gps_result', map, {error: 'gps_off'}));
        }
    }
}